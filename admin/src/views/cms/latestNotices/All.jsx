...
function All() {
  ...
  const HeaderNavigation = [
    {
      name: "All Notices",
      link: "/cms/latest-notices/all",
      icon: <FaRectangleList />,
    },
    {
      name: "Create Notice",
      link: "/cms/latest-notices/create",
      icon: <IoCreate />,
    },
    {
      name: "Trash Notices",
      link: "/cms/latest-notices/trash",
      icon: <FaTrashAlt />,
    },
  ];
  ...
  const fetchData = async () => {
    const response = await basicProvider.getMethod(
      `cms/latest-notice?page=${page}&count=${count}&search=${search}`
    );
            ...
            {multiDelete && multiDelete?.length > 0 && (
              <div className="w-full cp flex items-center justify-between text-2xl">
                <div style={{ fontSize: "16px" }}>
                  {multiDelete?.length} selected to Delete
                </div>
                <DeleteSweetalert
                  endpoint={"cms/latest-notice"}
                  type={"multi-trash"}
                    ...
                    <TableBody>
                      {data?.map((row) => (
                        ...
                        >
                          ...
                          <TableCell align="center">{createdAt(row.createdAt)}</TableCell>
                          <TableCell align="center">
                            <div className="flex items-center gap-1 text-2xl">
                              <NavLink to={`/cms/latest-notices/${row._id}/edit`}>
